...
int main(int argc, char *argv[])
{
    ...
    if(lw.exec() == QDialog::Accepted)
    {
        ...
        if(!Activation::tryIncrementLaunchCounter(lw.GetUserName(), activated))
        {
            qDebug() << "The program can't be launched due to trial limits";
            PLOGI    << "The program can't be launched due to trial limits";
            return 5;
        }
        ...
        MainWindow w(lw.GetUserName(), activated);
        w.show();
        return a.exec();
    }
    else
