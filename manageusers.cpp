...
{
    /* Перевіряємо, наявність та відсутність символів за допомогою regex (механызму, що працює на регулярних виразах). Вказуємо йому паттерн, що перевіряє відсутність пробілів (?!.*\\s), наявність хочаб 1 цифри (?=.*\\d), наявність хочаб 1 літери латинського алфавіту, наявність хочаб 1 літери кириличного алфавіту */
    static const QRegularExpression passwordRegex{ "(?!.*\\s)(?=.*\\d)(?=.*[[:alpha:]])(?=.*[А-Яа-яіІїЇЁё])" };
